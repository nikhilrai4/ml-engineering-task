...
def _is_dict_of_dicts(obj: Dict[str, Any]) -> bool:
    """Returns True if all top-level values are dicts ({col: {index: value}})."""
...
def _rows_from_dict_of_dicts(obj: Dict[str, Dict[str, Any]]) -> Iterable[Dict]:
    ...
    * Sorts indices numerically if they look like integers, else lexicographically
    """
...
def _rows_from_json_obj(obj: Any) -> Iterable[Dict]:
    ...
    - dict[str, dict]       -> column-oriented dicts (expand)  <-- pandas orient='columns'
...
def _normalize_item_id(value: Any) -> Any:
    """
    Ensure "item_id" is a scalar (preferred id-like key if present).
    """
    if isinstance(value, _SCALAR_TYPES):
        return value
...
class ProcessGroup(beam.DoFn):
    ...
    def __init__(self, method: str):
        self.method = method
        self.m_groups = Metrics.counter("processing", "groups_total")
        self.m_failed = Metrics.counter("processing", "groups_failed")
    ...
    def process(self, kv: Tuple[Any, Iterable[Dict]]) -> Iterable[Dict]:
        ...
        # Coerce numerics (parity with pandas read_json)
        for col in _NUMERIC_COLS:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors="coerce")

        out_df, failed = process(df, self.method)
        if failed:
            self.m_failed.inc()
...
class RecordsToSingleJsonString(beam.CombineFn):
    ...
    def extract_output(self, acc: List[Dict]) -> str:
        ...
        # Single-line JSON like pandas default (orient='columns')
...
def run_beam(
    ...
    direct_running_mode: str = "multi_threading",  # or 'multi_processing' (be careful if PyTorch spawns threads)
    direct_num_workers: int = 4,
    remaining_args=None,
) -> None:
    """
    Execute the Beam pipeline locally via DirectRunner.

    Args:
        method: one of {"full", "discount", "transactions", "legacy"}.
        pool_over: grouping key (e.g., "item_id", "lowest_category", "highest_category").
        input_path: path or glob to input JSON(s).
        output_path: output file (exact filename).
        direct_running_mode: "multi_threading" (default) or "multi_processing".
        direct_num_workers: number of worker threads or processes.
        remaining_args: additional pipeline args list (rarely needed for DirectRunner).
    """
    opts = PipelineOptions(
        remaining_args or [],
        runner="DirectRunner",
        save_main_session=True,  # needed when DoFns reference local modules (e.g., processing.process)
    )
    ...
    with beam.Pipeline(options=opts) as p:
        rows = (
            ...
            | "Parse JSON/NDJSON" >> beam.ParDo(ReadJsonFlexibleAsRows())
        ...
        (
            ...
            | "Write output.json" >> beam.io.WriteToText(
                output_path,
                num_shards=1,
                shard_name_template="",  # exact filename (no shard suffix)
            )
        )
